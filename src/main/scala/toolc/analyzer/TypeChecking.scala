    ...
    def tcExpr(expr: ExprTree, expectedTps: Type*): Unit = {
      expr match {
        ...
        case mc@MethodCall(obj, meth, _) =>
          if(mc.getType == TError) sys.error(s"Wrong arguments type in method call of method ${meth.value}")
          obj.getType match {
      ...
      if (!expectedTps.toList.exists(expr.getType.isSubTypeOf)) {
        error("Type error: Expected: " + expectedTps.mkString(" or ") + s", found: ${expr.getType} in ${expr.position}", expr)
      }
